...
global_params['z_takeoff'] = NumericalParam('z_takeoff', 0.0, 5.0, 1.0)
...
class TakeoffState(State):
    ...
    def execute(self):
        ...
        self.console.set_header('Taking off at 1 [m]...')
        self.console.set_footer('')
        self.console.reset_box()
        self.take_off_pub.publish()
        rospy.sleep(5)
        return 'quit'
    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            if keycode == ord('q'):
                ...
                vel_cmd = Twist()
                vy = +vel_mul * joy.axis['move_forward'].value
                vx = +vel_mul * joy.axis['move_right'].value
